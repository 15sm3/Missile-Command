...
void mouseToWorldCoordinates(double &xpos, double &ypos) {
  
  //Gets world coordinates from mouse position

  float worldWidth = WORLD_RIGHT - WORLD_LEFT;
  float worldHeight = worldTop - WORLD_BOTTOM;

  xpos = ((xpos - 12)/(float)WINDOW_WIDTH) * worldWidth;
  ypos = worldHeight - ((ypos + 10)/(float)WINDOW_HEIGHT) * worldHeight;
